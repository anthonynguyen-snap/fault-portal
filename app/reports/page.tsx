...
export default function ReportsPage() {
  ...
  function exportCsv() {
    ...
    const headers = isInternal
      ? ['Date','Order #','Customer','Product','Manufacturer','Fault Type','Fault Subtype','Taxonomy Status','Original Fault Type','Notes','Manufacturer Number','Evidence Link']
      : ['Date','Order #','Product','Manufacturer','Fault Type','Fault Subtype','Taxonomy Status','Original Fault Type','Notes','Manufacturer Number','Evidence Link'];
    const rows = filteredCases.map(c => isInternal
      ? [c.date, c.orderNumber, c.customerName, c.product, c.manufacturerName, c.faultType, c.faultSubtype || '', c.taxonomyStatus || '', c.originalFaultType || '', c.faultNotes, c.manufacturerNumber, c.evidenceLink || '']
      : [c.date, c.orderNumber, c.product, c.manufacturerName, c.faultType, c.faultSubtype || '', c.taxonomyStatus || '', c.originalFaultType || '', c.faultNotes, c.manufacturerNumber, c.evidenceLink || '']
    );
