...
function fromRow(row: Record<string, unknown>) {
  return {
    ...
    commslayerAgentId: String(row.commslayer_agent_id ?? ''),
  ...
  try {
    ...
    const { data, error } = await getSupabase()
      ...
      .upsert({
        ...
        commslayer_agent_id: String(body.commslayerAgentId ?? '').trim(),
        performance_primary: Boolean(body.performancePrimary),
