          ...
          <div className="card p-5">
                ...
                <div key={item.id} className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">Item {i + 1}</p>
                    </div>
                    <div className="flex flex-wrap gap-1.5 justify-end flex-shrink-0">
                      {badge(item.condition, conditionColour[item.condition] || 'bg-slate-100 text-slate-600')}
                      {badge(item.decision, decisionColour[item.decision] || 'bg-slate-100 text-slate-600')}
                    </div>
                  </div>
                  {item.refundAmount > 0 && (
                    <p className="text-sm font-semibold text-emerald-700 mt-2">Refund: ${item.refundAmount.toFixed(2)}</p>
                  )}
                  {item.restockingFee > 0 && (
                    <p className="text-xs text-amber-600 mt-1">{item.restockingFee}% restocking fee</p>
                  )}
                </div>
              ))}
            </div>
            {data.totalRefundAmount > 0 && (
              <div className="mt-3 pt-3 border-t border-slate-100 flex justify-between items-center">
                ...
                <span className="text-base font-bold text-emerald-700">${data.totalRefundAmount.toFixed(2)}</span>
              </div>
            )}
          </div>
